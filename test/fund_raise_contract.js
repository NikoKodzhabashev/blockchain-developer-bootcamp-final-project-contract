...
async function getTransactionGasCost(tx) {
    ...
    return web3.utils.toBN(parseInt(transaction.gasPrice) * parseInt(transactionReceipt.gasUsed));
}
...
async function createFreshCampaign(fundRaiseContract, account) {
    ...
    await fundRaiseContract.createCampaign(expireOf, goal, title, description, ipfsHash, { from: account });
...
contract("FundRaiseContract", function (accounts) {
    ...
    describe("createCampaign", () => {
        it("should create a new campaign", async () => {
            const { goal, expireOf, title, description, ipfsHash } = await createFreshCampaign(fundRaiseContract, fundraiser);
            ...
            assert.equal(parseInt(campaign.currentAmount.toString()), 0, "Mismatch currentAmount");
            assert.equal(parseInt(campaign.expireOf.toString()), parseInt(blockTimestamp.toString()) + parseInt(expireOf.toString()), "Mismatch expireOf");
    ...
    describe("donate", () => {
        it("should increase the campaign balance on donate when the campaign is active", async () => {
            await createFreshCampaign(fundRaiseContract, fundraiser);
            await fundRaiseContract.donate(campaignId, { from: donator, value: donation });

            const updatedCampaign = await fundRaiseContract.campaigns(campaignId);

            assert.equal(parseInt(updatedCampaign.currentAmount.toString()), donation, "Current amount not equal to the donation");
        });
        it("should complete the campaign when donate to expired campaign", async () => {
            ...
            assert.equal(parseInt(updatedCampaign.status.toString()), FundRaiseContract.FundRaiseStatus.Completed, "Wrong status");
            assert.equal(parseInt(updatedCampaign.currentAmount.toString()), parseInt(campaign.currentAmount.toString()), "Amount should not be changed");
        });
    });
    ...
    describe("withdraw", () => {
        it("should allow the campaign creator to withdraw the money and complete the campaign when it is active", async () => {
            ...
            const fundraiserBalanceBeforeWithdraw = await web3.eth.getBalance(fundraiser);

            const receipt = await fundRaiseContract.withdraw(campaignId, { from: fundraiser });
            const gasCost = await getTransactionGasCost(receipt.tx);

            const fundraiserBalanceAfterWithdraw = await web3.eth.getBalance(fundraiser);

            const updatedCampaign = await fundRaiseContract.campaigns(campaignId);

            assert.equal(
                parseInt(fundraiserBalanceAfterWithdraw),
                parseInt(fundraiserBalanceBeforeWithdraw) + donation - parseInt(gasCost.toString()),
                "Wrong ballance after withdraw"
            );
            assert.equal(parseInt(updatedCampaign.status.toString()), FundRaiseContract.FundRaiseStatus.Completed);
        ...
        it("should not allow non-campaign creator to withdraw the money", async () => {
            ...
            await expectRevert(fundRaiseContract.withdraw(campaignId, { from: maliciousWithdrawer }), "Withdrawer not authorized.");
        });
        it("should not allow to withdraw the money when the campaign is active", async () => {
            await createFreshCampaign(fundRaiseContract, fundraiser);

            await expectRevert(fundRaiseContract.withdraw(campaignId, { from: fundraiser }), "Can't withdraw since the campaigns is still active.");
        });
    });

    it.only("it should return all campaigns of an address", async () => {
        const { goal, expireOf, title, description, ipfsHash } = await createFreshCampaign(fundRaiseContract, fundraiser);
        const fundraiserCampaigns = await fundRaiseContract.getAllCampaignsByAddress();
        const latestTime = await time.latest();

        const calculatedExpireOf = (parseInt(expireOf.toString()) + parseInt(latestTime.toString())).toString();
        console.log(calculatedExpireOf, "HERE");
        expect(fundraiserCampaigns).to.deep.equal([["0", goal.toString(), "0", calculatedExpireOf, description, title, ipfsHash, "0"]]);
    });
